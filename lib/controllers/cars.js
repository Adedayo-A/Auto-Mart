...
const jwt = require('jsonwebtoken'); // GET REQUESTS
...
const getCars = (req, res) => {
  ...
  jwt.verify(req.token, process.env.JWT_KEY, (err, authData) => {
    if (err) {
      ...
      const value = [email]; // eslint-disable-next-line consistent-return
...
const postCar = (req, res) => {
  ...
  jwt.verify(req.token, process.env.JWT_KEY, (err, authData) => {
    if (err) {
      ...
      pg.connect();
      let query = 'SELECT * FROM users WHERE LOWER(email) = LOWER($1)';
      let value = [email]; // eslint-disable-next-line consistent-return
